...
<div class="max-w-5xl mx-auto p-6">
    ...
    <div class="space-y-6">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
            ...
            <div class="px-6 py-4 grid grid-cols-1 md:grid-cols-[200px,1fr] items-start gap-6">
                <div class="flex flex-col items-center gap-3">
                    <img src="{{ $avatarUrl }}" alt="Avatar" style="height:20%; width:auto;" class="rounded-full object-cover ring-1 ring-gray-200" />
                </div>
                <div class="form-group mb-2">
                    ...
                    <div class="flex items-center gap-3">
                        <button wire:click="updateAvatar" wire:loading.attr="disabled" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
                        ...
                        <button wire:click="deleteAvatar" type="button" class="px-4 py-2 border border-gray-300 rounded-md hover:bg-gray-50">Eliminar foto</button>
                        @if ($avatar)
                            <span class="text-sm text-gray-500">Archivo seleccionado: {{ $avatar->getClientOriginalName() }}</span>
                        @endif
                    </div>
                    </div>
            </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
            ...
            <div class="px-6 py-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" wire:model.blur="fields.name" class="px-2 mt-1 w-full rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500" />
                    @error('fields.name')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700">Apellido</label>
                    <input type="text" wire:model.blur="fields.lastname" class="px-2 mt-1 w-full rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500" />
                    @error('fields.lastname')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700">Correo electrónico</label>
                    <input type="email" wire:model.blur="fields.email" class="px-2 mt-1 w-full rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500" />
                    @error('fields.email')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700">Usuario</label>
                    <input type="text" wire:model.blur="fields.username" class="px-2 mt-1 w-full rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500" />
            ...
            <div class="px-6 py-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div class="md:col-span-2 grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="password" wire:model.defer="passwords.current" class="px-2 mt-1 w-full rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500" />
                        @error('passwords.current')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700">Nueva contraseña</label>
                        <input id="new-password" type="password" wire:model.defer="passwords.new" class="px-2 mt-1 w-full rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500" />
                        @error('passwords.new')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                        <p class="text-xs text-gray-500 mt-1">Mín. 8 caracteres, con mayúsculas, minúsculas, número y símbolo.</p>
                        <div class="mt-2">
                            <div class="w-full h-2 bg-gray-200 rounded">
                                <div id="pw-strength-bar" class="h-2 w-0 rounded" style="background-color:#ef4444"></div>
                            </div>
                    ...
                    <div>
                        ...
                        <input type="password" wire:model.defer="passwords.confirm" class="px-2 mt-1 w-full rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500" />
                        @error('passwords.confirm')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                    </div>
                </div>
                <div class="md:col-span-2">
                    <button wire:click="updatePassword" wire:loading.attr="disabled" class="px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
        ...
        <div class="bg-white rounded-xl border border-gray-200 shadow-sm">
            ...
            <div class="px-6 py-4 grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="text" wire:model.blur="fields.phone" class="px-2 mt-1 w-full rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500" />
                    @error('fields.phone')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700">Tipo de documento</label>
                    <input type="text" wire:model.blur="fields.document_type" class="px-2 mt-1 w-full rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500" />
                    @error('fields.document_type')<p class="text-sm text-red-600 mt-1">{{ $message }}</p>@enderror
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700">Número de documento</label>
                    <input type="text" wire:model.blur="fields.document_number" class="px-2 mt-1 w-full rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500" />
                ...
                <div>
                    ...
                    <input type="text" wire:model.blur="fields.institution" class="px-2 mt-1 w-full rounded-md border border-gray-300 focus:ring-blue-500 focus:border-blue-500" />
        ...
        <div class="flex items-center gap-3">
            <button wire:click="update" wire:loading.attr="disabled" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-md hover:bg-blue-700">
                <span wire:loading.remove>Actualizar información personal</span>
                <span wire:loading>Guardando...</span>
            </button>
            <button wire:click="resetFields" type="button" class="px-4 py-2 rounded-md border border-gray-300 hover:bg-gray-50">Cancelar</button>
        </div>
    </div>
</div>
@push('scripts')
